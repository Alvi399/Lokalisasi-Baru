...
#include "vision_utils/Point2D.h"
#include "vision_utils/PointArray.h"
...
private:
   ...
   // Eigen::Quaterniond data_imu;
   // void imuDataCb(const sensor_msgs::Imu::ConstPtr &_msg){

   //      data_imu.x() = _msg->orientation.x;
   //      data_imu.y() = _msg->orientation.y;
   //      data_imu.z() = _msg->orientation.z;
   //      data_imu.w() = _msg->orientation.w;
   // }
   ...
   //  }
    ros::Subscriber angle_data_sub;
    double angle_pitch;
    double angle_roll;
    double angle_yaw;
    void angleDataCb(const geometry_msgs::PointConstPtr &_msg)
    {
        angle_roll = (_msg->x) * 180 / Math::PI;
        angle_pitch = (_msg->y) * 180 / Math::PI;
        double current_pos = ((_msg->z) * 180 / Math::PI) > 0 ? ((_msg->z) * 180 / Math::PI) : 360 + ((_msg->z) * 180 / Math::PI);
        ;
...
//#endif
    ...
    //PROYEKSI PUNYA V12
    // double ngitungJarak(float diameterPixel);
    // Points ngitungJarak(const Points &_points, float diameterPixel, bool ball);
    ...
    // geometry_msgs::Point ball_pos_;
    // void diameterPixelCb(const geometry_msgs::PointConstPtr &_msg){
    //     ball_pos_.x = _msg->x;
    //     ball_pos_.y = _msg->y;
    //     ball_pos_.z = _msg->z;
    // }
    ...
    void odometryCb(const geometry_msgs::PoseStampedConstPtr &_msg){
        // odometer_out_.x = _msg->pose.position.x <= -100.0 ? .0 : _msg->pose.position.x * 100.0;
        // odometer_out_.y = _msg->pose.position.y <= -100.0 ? .0 : _msg->pose.position.y * 100.0;
        odometer_out_.x = _msg->pose.position.x;
        odometer_out_.y = _msg->pose.position.y;
    ...
    // float getHeadAngle();
    // void head_angle_Cb(const std_msgs::Float64 &head_angle_){
    //     head_angle_ = head_angle_.data;
    // }
    //data yg diperlukan 
    Points target_points_real;

    //target_points callback 
    void targetPointsCb(const vision_utils::PointArray &data_points);

    //traget points subscribe 
