...
Localization::Localization():nh_(ros::this_node::getName()),it_(nh_),
    ...
    js_sub_(nh_,"/robotis/goal_joint_states",1),
    #ifdef GAZEBO
    imu_sub_(nh_,"/robotis_op3/imu",1),
    #else
    imu_sub_(nh_,"/robotis/open_cr/imu",1),
    #endif
    ...
    //sudut kepala
    head_angle_pub_(nh_.advertise<std_msgs::Float64>("head_angle_", 10)),
    ...
    camera_info_sub_(nh_.subscribe("/robotis_op3/camera/camera_info",1,&Localization::cameraInfoCallback
        ,this)),
    #else
    ...
    jarak_X_sub_(nh_.subscribe("/v9_ball_detector_node/jarak_X",1,&Localization::jarak_X_Cb,this)),
    target_points_sub_(nh_.subscribe("/v9_ball_detector_node/target_points",1,&Localization::targetPointsCb, this)),
    ...
    angle_data_sub(nh_.subscribe("/alfarobi/angle", 1, &Localization::angleDataCb, this)),
...
void Localization::targetPointsCb(const vision_utils::PointArray &msg) {
    target_points_real.clear();  // Kosongkan vector sebelum mengisi ulang

    // Konversi dari ROS message ke std::vector<cv::Point>
    for (const auto& p : msg.points) {
        target_points_real.emplace_back(p.x, p.y);
    }
}
...
bool Localization::setInputImage(){
    // std::cout << "cv_sw_ptr: " << cv_sw_ptr_sub_ << "    : cv_isg_ptr" <<  cv_isg_ptr_sub_ << std::endl;
...
inline float Localization::verticalDistance(float _tilt_dev){
    ...
    // std::cout << "[VISION] Tilt : " << (Math::PI_TWO - (CAMERA_ORIENTATION.coeff(1))) * (180/Math::PI) << std::endl;
    std_msgs::Float64 head_angle_;
    head_angle = (Math::PI_TWO - (CAMERA_ORIENTATION.coeff(1))) * 180/Math::PI;
    // head_angle = 69;
    head_angle_.data = head_angle;
    head_angle_pub_.publish(head_angle_); 
    // std::cout << "[VISION] Height : " << (robot_height_ + CAMERA_DIRECTION.coeff(2) + z_offset_) << std::endl;   
...
// //PROYEKSI PUNYA V12 NJING
// double Localization::ngitungJarak(float diameterPixel){
//     double jarakX = (realDiameter * focalLength)/diameterPixel;
//     return jarakX;
...
// }
//PROYEKSI PUNYA V12 NJING
// Points Localization::ngitungJarak(const Points &_points, float diameterPixel, bool ball){
...
//    const float PI_2 = CV_PI/2;
    ...
    // #ifdef STATE
    std::cout<<"[VISION] Pan   : "<<((pan_servo_angle_ + pan_rot_comp_)*180/CV_PI)<<"\n";
    std::cout<<"[VISION] Roll : " << (CAMERA_ORIENTATION.coeff(0)*180/CV_PI) <<"\n\n";
    // #endif
...
//        float offset_tilt = tiltAngleDeviation(undistort_y);
        ...
        // std::cout<<"TILT  : "<<(Math::PI_TWO - (CAMERA_ORIENTATION.coeff(1) + offset_tilt))*(180/Math::PI)<<std::endl;
...
//baru v12
    static std::vector<double> robot_state_ma_x, robot_state_ma_y, robot_state_ma_z;
    geometry_msgs::Pose2D robot_state_filt;
    int size_buff_robot_state = 5;
                
    robot_state_ma_x.insert(robot_state_ma_x.end(), robot_state_.x);
    robot_state_ma_y.insert(robot_state_ma_y.end(), robot_state_.y);
    robot_state_ma_z.insert(robot_state_ma_z.end(), robot_state_.theta);

    // std::cout<<"STATE MA X ASUUU: "<<robot_state_ma_x.size()<<std::endl;   
    // std::cout<<"STATE MA Y ASUUU: "<<robot_state_ma_y.size()<<std::endl;   
    if (robot_state_ma_y.size() < size_buff_robot_state)
    {               
        robot_state_filt.x = double(std::accumulate(robot_state_ma_x.begin(),
                                    robot_state_ma_x.end(), 0) / robot_state_ma_x.size());
        robot_state_filt.y = double(std::accumulate(robot_state_ma_y.begin(),
                                    robot_state_ma_y.end(), 0) /robot_state_ma_y.size());
        robot_state_filt.theta = double(std::accumulate(robot_state_ma_z.begin(),
                                    robot_state_ma_z.end(), 0) / robot_state_ma_z.size());
        #ifdef STATE
        ROS_INFO("MASUKKKK IFFFF");
        #endif
    }
    else
    {
        robot_state_filt.x = double(std::accumulate(robot_state_ma_x.begin(),
                                    robot_state_ma_x.end(), 0) / size_buff_robot_state);
        robot_state_ma_x.erase(robot_state_ma_x.begin());

        robot_state_filt.y = double(std::accumulate(robot_state_ma_y.begin(),
                                    robot_state_ma_y.end(), 0) /size_buff_robot_state);
        robot_state_ma_y.erase(robot_state_ma_y.begin());

        robot_state_filt.theta = double(std::accumulate(robot_state_ma_z.begin(),
                                    robot_state_ma_z.end(), 0) / size_buff_robot_state);
        robot_state_ma_z.erase(robot_state_ma_z.begin());
        #ifdef STATE
        ROS_INFO("MASUK ELSEEEE");
        #endif
    }
    robot_state_msg.pose.position.x = robot_state_filt.x ;
    robot_state_msg.pose.position.y = robot_state_filt.y ;
    robot_state_msg.pose.orientation.z = robot_state_filt.theta;
    
    // robot_state_msg.pose.position.x = robot_state_.x ;
    // robot_state_msg.pose.position.y = robot_state_.y ;
    // robot_state_msg.pose.orientation.z = angle_yaw;
    ...
    if(ball_pos_.x != -1.0 &&
        ball_pos_.y != -1.0){
        // std::cout << "BALL_POS_.X ====== " << ball_pos_.x << std::endl;
        // std::cout << "BALL_POS_.Y ====== " << ball_pos_.y << std::endl;  
        //baru v12
        int jarakX_int = int(jarakX);
        Points ball_pos_x;
        ball_pos_x.emplace_back(cv::Point(0, jarakX_int));

        Points ball_pos;
        ball_pos.emplace_back(cv::Point(ball_pos_.x,jarakX_int));
        

        // ball_pos = fastPointsProjection(ball_pos, true);
        ball_pos = pointsProjection(ball_pos, true); //y sama z
        // ball_pos2 = ball_pos;
        // ball_pos2 = ngitungJarak(ball_pos2, diameterPixel, true);  // x doang
        // ball_posngitungJarak(ball_pos, true);
         // ball_pos = ngitungJarak(ball_pos);
        ...
        if(ball_pos.size() > 0){
            ...
            //anyar
            ball_pos_msg.x = ball_pos_x.front().y;
            // ball_pos_msg.x = ball_pos.front().y;
            ball_pos_msg.y = -ball_pos.front().x;
            ball_pos_msg.z = shoot_dir;
            projected_ball_pub_.publish(ball_pos_msg);

            // if (ball_pos.front().y < 25)
            // {
            //     ball_pos_msg.x = ball_pos_x.front().y;
            //     ball_pos_msg.y = -ball_pos.front().x;
            //     ball_pos_msg.z = shoot_dir;
            ...
            //diganti baru
            proj_ball_stamped_msg.point.x = ball_pos_x.front().x;
    ...
    //             static std::vector<double> robot_state_msg_ma_x, robot_state_msg_ma_y, robot_state_msg_ma_z;
    //             geometry_msgs::PoseStamped robot_state_msg_filt;
    //             double robot_state_msg_filt_x, robot_state_msg_filt_y, robot_state_msg_filt_z;
    //             int size_buff_robot_state = 10;
                
    //             robot_state_msg_ma_x.insert(robot_state_msg_ma_x.end(), robot_state_msg.pose.position.x);
    //             robot_state_msg_ma_y.insert(robot_state_msg_ma_y.end(), robot_state_msg.pose.position.y);
    //             robot_state_msg_ma_z.insert(robot_state_msg_ma_z.end(), robot_state_msg.pose.orientation.z);

                
    //             if (robot_state_msg_ma_x.size() < size_buff_robot_state)
    //             {
                    
    //                 robot_state_msg_filt_x = double(std::accumulate(robot_state_msg_ma_x.begin(),
    //                                 robot_state_msg_ma_x.end(), 0) / robot_state_msg_ma_x.size());
    //                 robot_state_msg_filt_y = double(std::accumulate(robot_state_msg_ma_y.begin(),
    //                                 robot_state_msg_ma_y.end(), 0) /robot_state_msg_ma_y.size());
    //                 robot_state_msg_filt_z = double(std::accumulate(robot_state_msg_ma_z.begin(),
    //                                 robot_state_msg_ma_x.end(), 0) / robot_state_msg_ma_x.size());
    //             }
    //             else
    //             {
    //                 robot_state_msg_filt_x = double(std::accumulate(robot_state_msg_ma_x.begin(),
    //                                 robot_state_msg_ma_x.end(), 0) / size_buff_robot_state);
    //                 robot_state_msg_ma_x.erase(robot_state_msg_ma_x.begin());

    //                 robot_state_msg_filt_y = double(std::accumulate(robot_state_msg_ma_y.begin(),
    //                                 robot_state_msg_ma_y.end(), 0) /size_buff_robot_state);
    //                 robot_state_msg_ma_y.erase(robot_state_msg_ma_y.begin());

    //                 robot_state_msg_filt_z = double(std::accumulate(robot_state_msg_ma_z.begin(),
    //                                 robot_state_msg_ma_x.end(), 0) / size_buff_robot_state);
    //                 robot_state_msg_ma_z.erase(robot_state_msg_ma_z.begin());
    //             }

                    
    //                 robot_state_msg_filt.pose.position.x = robot_state_msg_filt_x;
    //                 robot_state_msg_filt.pose.position.y = robot_state_msg_filt_y ;
    //                 robot_state_msg_filt.pose.orientation.z = robot_state_msg_filt_z;
    //                 robot_state_msg_filt.header.stamp = this->stamp_;
    //                 robot_state_msg_filt.header.seq++;
    
    // std::cout << "[LOCALIZATION] ROBOT X : " << robot_state_msg_filt.pose.position.x << std::endl;
    // std::cout << "[LOCALIZATION] ROBOT Y : " << robot_state_msg_filt.pose.position.y << std::endl;
    // std::cout << "[LOCALIZATION] ROBOT THETA : " << robot_state_msg_filt.pose.orientation.z << std::endl;

    // std::cout << "[LOCALIZATION] ROBOT X : " << robot_state_msg.pose.position.x << std::endl;
    // std::cout << "[LOCALIZATION] ROBOT Y : " << robot_state_msg.pose.position.y << std::endl;
    // std::cout << "[LOCALIZATION] ROBOT THETA : " << robot_state_msg.pose.orientation.z << std::endl;
...
//    cv::imshow("SW",segmented_white_);
    ...
    std::cout << "tilt_servo_angle: " << tilt_servo_angle_ << " offset_head: " << offset_head_ << " roll_offset: " << roll_offset_ << std::endl;
    if(tilt_servo_angle_ < tilt_limit_ && offset_head_ < 40.0f && fabs(roll_offset_) < 22.0f){ //robot condition for feature observation
        std::cout << "MASUK PROSES LOKALISASI" << std::endl;
        Points target_points = target_points_real;
...
//        LocalizationUtils::getInstance()->adaptiveScanLinePoints(invert_green_,segmented_white_,tilt_servo_angle_,pan_servo_angle_,field_boundary_,target_points);
    ...
    // std_msgs::Float64 head_angle_;
    // head_angle = (Math::PI_TWO - (CAMERA_ORIENTATION.coeff(1)));
    // // head_angle = 69;
    // head_angle_.data = head_angle;
    // head_angle_pub_.publish(head_angle_); // std_msgs::Float64 head_angle_;
        // head_angle_.data = head_angle;
        // head_angle_pub_.publish(head_angle_);
